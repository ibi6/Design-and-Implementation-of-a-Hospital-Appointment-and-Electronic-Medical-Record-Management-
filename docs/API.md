...
| 名称 | 方法与地址 | 请求 | 返回 | 权限 |
|---|---|---|---|---|
| CSRF Token | `GET /auth/csrf` | 无 | `headerName`, `token`；同时写 `XSRF-TOKEN` Cookie | 公开 |
...
浏览器前端会自动获取并携带 CSRF Token。PowerShell 直接调用 Cookie 登录时可使用：

```powershell
$session = [Microsoft.PowerShell.Commands.WebRequestSession]::new()
$csrf = Invoke-RestMethod http://localhost:8080/api/auth/csrf -WebSession $session
$headers = @{}
$headers[$csrf.data.headerName] = $csrf.data.token
$body = @{ username = 'patient'; password = '123456' } | ConvertTo-Json
Invoke-RestMethod http://localhost:8080/api/auth/login -Method Post -WebSession $session -Headers $headers -ContentType 'application/json' -Body $body
Invoke-RestMethod http://localhost:8080/api/auth/me -WebSession $session
```

使用 `Authorization: Bearer <jwt>` 的非浏览器 API 写请求不需要 CSRF Token。
